  ...
  public class HTTPRequest
  {
    ...
    public HTTPRequest(String request)
    {
      ...
      for(int i = 1; i != lines.Length; i++) {
        String[] pair = Regex.Split(lines[i], ":"); 
        if (pair.Length == 0) continue;
       if (pair.Length==3) // still wrong
                {
                        String[] pair2 = Regex.Split(lines[i], "accepted");
                        addProperty("ip", pair2[0]);
                }
       else if(pair.Length == 1) { // handle post body
